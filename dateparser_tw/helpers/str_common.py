    ...
    if match is not None:
        ...
        if match is None:
            ...
            if match is not None:
                ...
                input_query = input_query[:end] + "號" + input_query[end:]
    # 一個半小时
    pattern = re.compile(r"(.*半)(?=(小時|月))")
    match = pattern.search(input_query)
    if match is not None:
        test_ge = re.compile(r"(.*)(?=個半)")
        ...
        if match.group() == "半":
            ...
            input_query = input_query.replace(test_ge_match.group() + "個半", number)

    pattern = re.compile(r"小時|月")
    match = pattern.search(input_query)
    if match is None:
        input_query = input_query.replace("個", "")

    input_query = input_query.replace("中旬", "15號")
    input_query = input_query.replace("傍晚", "午後")
    input_query = input_query.replace("大年", "")
    input_query = input_query.replace("新年", "春節")
    input_query = input_query.replace("五一", "勞動節")
...
def number_translator(target):
    ...
    pattern = re.compile(
        r"[一二兩三四五六七八九123456789]萬[一二兩三四五六七八九123456789](?!(千|百|十))"
    )
    ...
    pattern = re.compile(
        r"[一二兩三四五六七八九123456789]千[一二兩三四五六七八九123456789](?!(百|十))"
    )
    match = pattern.finditer(target)
    for m in match:
        group = m.group()
    ...
    pattern = re.compile(
        r"[一二兩三四五六七八九123456789]百[一二兩三四五六七八九123456789](?!十)"
    )
    match = pattern.finditer(target)
    for m in match:
        group = m.group()
    ...
    pattern = re.compile(r"[零一二兩三四五六七八九]")
    match = pattern.finditer(target)
    for m in match:
        target = pattern.sub(str(word2number(m.group())), target, 1)

    # 星期天表达式替换为星期7
    pattern = re.compile("(?<=(周|週|星期|禮拜))[末天日]")
    match = pattern.finditer(target)
    for m in match:
        target = pattern.sub("7", target, 1)

    pattern = re.compile("(?<!(周|週|星期|禮拜))0?[0-9]?十[0-9]?")
    match = pattern.finditer(target)
    for m in match:
        group = m.group()
        s = group.split("十")
